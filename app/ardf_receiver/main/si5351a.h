...
class Si5351A {
 public:
  ...
  static constexpr uint8_t kRDivBitsMax = 7;  // R分周器のレジスタ値の最大 (2^7 = 128)
  ...
  // 0x4F = 0b01001111: Power Up, Integer Mode, PLLA, Not Inverted, MultiSynth, 8mA
  static constexpr uint8_t kClkPower8mA = 0x4F;  // Integer mode + PLLA + MultiSynth + 8mA
  static constexpr uint8_t kClkPower6mA = 0x4E;  // Integer mode + PLLA + MultiSynth + 6mA
  static constexpr uint8_t kClkPower4mA = 0x4D;  // Integer mode + PLLA + MultiSynth + 4mA
  static constexpr uint8_t kClkPower2mA = 0x4C;  // Integer mode + PLLA + MultiSynth + 2mA
 ...
 private:
  ...
  void SetPLL(const uint8_t pll, const uint8_t mult, const uint32_t num, const uint32_t denom);
  void SetMultiSynth(const uint8_t synth, const uint32_t div, const uint8_t rdiv_bits);
