...
MCP4018::MCP4018() : dev_handle_(nullptr) {}
...
void MCP4018::Setup() {
  ...
  util::SleepMillisecond(hardware_config::kMcp4018PowerOnDelayMs);
...
void MCP4018::SetWiper(uint8_t value) {
  // 値の範囲チェック
  if (value > kWiperMax) {
    ESP_LOGW(kTag, "Wiper value %u exceeds max %u, clamping to max", value,
             kWiperMax);
    value = kWiperMax;
  }

  ESP_LOGD(kTag, "Setting wiper to %u (0=max resistance, 127=min resistance)",
           value);
...
void MCP4018::Write(const uint8_t value) {
  ...
  uint8_t write_buf = value & kWiperWriteMask;
