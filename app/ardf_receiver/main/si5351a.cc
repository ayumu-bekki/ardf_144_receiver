...
void Si5351A::Setup(const uint8_t address) {
  ...
  clk_output_status_ = 0xff;

  ESP_LOGI(kTag, "Si5351A initializing at address 0x%02X", address);
  Write(kRegOutputCtrl, clk_output_status_); // 全出力を無効化
  Write(kRegClk0Ctrl, 0x80); // CLK0をパワーダウン
  Write(kRegClk1Ctrl, 0x80); // CLK1をパワーダウン
  Write(kRegClk2Ctrl, 0x80); // CLK2をパワーダウン
  Write(kRegXtalLoad, 0x80); // 水晶振動子の負荷容量を8pFに変更
  ESP_LOGI(kTag, "Si5351A initialized");
}

void Si5351A::SetFrequency(const Clk clk, const uint32_t freq) {
  // PLL分数分周器の分母 (最大分解能 20bit)
  constexpr uint32_t denom = 0x000fffff;
  ...
  while ((kPllFreqMin / target_freq) > kMultiSynthDivMax && rdiv_bits < kRDivBitsMax) {
    rdiv <<= 1;
    rdiv_bits++;
  ...
  for (uint32_t div = kMultiSynthDivMin; div <= kMultiSynthDivMax; div += step) {
    ...
    if (pll_freq >= kPllFreqMin && pll_freq <= kPllFreqMax) {
      ...
      const uint32_t error = (actual_freq > freq) ? (actual_freq - freq) : (freq - actual_freq);
  ...
  // 分子 b
  const uint32_t pll_num = (pll_freq % kXtalFreq) *
                           (static_cast<double>(denom) / kXtalFreq);

  // レジスタアドレスの決定
  const uint8_t reg_clk_ctrl = kRegClk0Ctrl + static_cast<uint8_t>(clk);
  const uint8_t reg_multisynth = kRegMs0 + static_cast<uint8_t>(clk) * 0x08;
...
void Si5351A::Write(const uint8_t reg, const uint8_t value) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(kTag, "Si5351A write failed (reg=0x%02X): %s", reg, esp_err_to_name(ret));
  }
}
...
void Si5351A::SetPLL(const uint8_t pll, const uint8_t mult, const uint32_t num, const uint32_t denom) {
  ESP_LOGD(kTag, "SetPLL: mult=%u, num=%u, denom=%u (%.6f MHz)",
           mult, num, denom, kXtalFreq * (mult + (double)num / denom) / 1e6);

  // レジスタ形式への変換（Si5351Aデータシート参照）
  const uint32_t P1 = (128 * mult) + (128.0 * static_cast<double>(num) / static_cast<double>(denom)) - 512;
  const uint32_t P2 = (128 * num) - (denom * static_cast<uint32_t>(128.0 * static_cast<double>(num) / static_cast<double>(denom)));
...
void Si5351A::SetMultiSynth(const uint8_t synth, const uint32_t div, const uint8_t rdiv_bits) {
  ...
  if (div == 4) {
    divby4_bits = 0x0C;  // 0b00001100 = bit 3-2を11bに設定
    ESP_LOGD(kTag, "Using DIVBY4 mode for div=4");
  }

  ESP_LOGD(kTag, "MS Registers: P1=%u, P2=%u, P3=%u, DIVBY4=0x%02X", P1, P2, P3, divby4_bits);

  // レジスタに書き込み
  Write(synth + 0, (P3 & 0x0000ff00) >> 8);
  Write(synth + 1, (P3 & 0x000000ff));
  // synth + 2 レジスタには R分周器(bit 6-4)、DIVBY4(bit 3-2)、P1の上位ビット(bit 1-0)が含まれる
  Write(synth + 2, ((rdiv_bits & 0x07) << 4) | divby4_bits | ((P1 & 0x00030000) >> 16));
